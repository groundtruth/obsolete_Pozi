...
try {
	$sql = sanitizeSQL("SELECT p.ezi_address as label,p.prop_propnum as prop_num FROM vw_dse_property_address As p WHERE ST_DWithin(p.the_geom, ST_Transform(ST_SetSRID(ST_Point(".$longitude.",".$latitude."),4326),4283), 0.001) ORDER BY ST_Distance(p.the_geom,ST_Transform(ST_SetSRID(ST_Point(".$longitude.", ".$latitude."),4326),4283)) limit ".$limit);
	$pgconn = pgConnection();
