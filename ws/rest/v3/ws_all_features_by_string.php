...
try {
	$sql = sanitizeSQL("select (case ld when 'Parcel SPI' then replace(label,'\\\','\\\\\\\') else initcap(label) end) as label,xmini,ymini,xmaxi,ymaxi,gsns,gsln,idcol,(case ld when 'Parcel SPI' then replace(idval,'\\\','\\\\\\\') else idval end) as idval,ld from gt_master_search where label like '%'||upper('".$query."')||'%' order by position(upper('".$query."') in label),(case when ld in ('Locality') then 1 when ld in ('Feature') then 2 when ld in ('Road') then 3 when ld in ('Address') then 4 else 9 end),label limit ".$limit);
	$pgconn = pgConnection();
