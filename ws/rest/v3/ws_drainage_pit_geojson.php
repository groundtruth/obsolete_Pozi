...
//	$sql = sanitizeSQL("SELECT t.id,t.prop_num,a.ezi_add as add_label,comments,t.ts,t.haz_type,t.haz_status,ST_AsGeoJSON(ST_Transform(t.the_geom,900913),6) as the_geom FROM (select * from msc_capture m ORDER BY ST_Distance(m.the_geom,ST_SetSRID(ST_Point(".$longitude.",".$latitude."),4326)) limit ".$limit.") t, dse_property_address a WHERE t.prop_num=a.pr_propnum and a.lga_code='".$lga."'");
	$sql = sanitizeSQL("SELECT t.ogc_fid as id,assetid as a_id,assetdescription as a_desc,typeid as tid,parentcode as p_code,(select case when count(*)>0 then 1 else 2 end from wsc_drainage_pit_cleaning_event e where e.pit_id=assetid) as i_status,ST_AsGeoJSON(ST_Transform(st_centroid(t.the_geom),900913),6) as the_geom FROM (select * from wsc_drainage_pit m ORDER BY ST_Distance(m.the_geom,ST_SetSRID(ST_Point(".$longitude.",".$latitude."),4326)) limit ".$limit.") t");
//	echo $sql;
	$pgconn = pgConnection();
